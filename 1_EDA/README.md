...
Introduction explaining what the project is and tools/skill implemented
...
- **Highest paid:** *Whick skills command the highest salaries?*  
...
- **Fact Table:** `job_postings_fact` - Central table containing job posting detail (job titles, locations, salaries, dates, etc.)
- **Dimension Tables:**
  - `company_dim` - Company information linked to job postings (not used in these three queries)
...
3. **[Optimal Skills](./03_optimal_skills.sql)** - Calculates an optimal score using percentile ranks of demand and median salary, combined through a weighted average, to identify the most valuable skills to learn
...
- **Complex Joins:** Multi-tables `INNER JOIN` operations across `job_postings_fact`, `skills_job_dim`, and `skills_dim`
...
- **Window Functions:** `PERCENT_RANK() OVER()` to convert raw salary and demand counts into comparable 0-1 percentile scores; `COUNT() OVER()` to compute each skill's share of total demand
- **Calculated Metrics:** Weighted composite scores (e.g. `0.9 * demand_percentile + 0.1 * salary_percentile`) to rank skills by a tunable blend of pay and demand
- **HAVING Clause:** Filtering aggregated results (skills with over 100 postings) to avoid small-sample noise
